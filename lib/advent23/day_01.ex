defmodule Advent23.Day01 do
  alias Advent23.Parse
  ...
  def solution(:second, input) do
    ...
    |> Enum.map(&calibration_value_from_integers/1)
    |> Enum.sum()
  end
  ...
  def solution(:first, input) do
    ...
    |> input()
    |> Parse.lines()
    |> Enum.map(&calibration_value_from_string/1)
    |> Enum.sum()
  end

  ## Private

  defp calibration_value_from_string(string) do
    string
    |> Parse.characters()
    |> Parse.as_integers()
    |> calibration_value_from_integers()
  end

  defp calibration_value_from_integers(integers) do
    first = List.first(integers)
    last = List.last(integers)

    (first * 10) + last
  end

  defp extract_numbers(string, acc \\ [])

  defp extract_numbers("", acc) do
    acc
    |> Enum.reject(& &1 == :error)
    |> Enum.reverse()
  end

  defp extract_numbers(<<_, rest::binary>> = string, acc) do
    digit =
      Enum.find_value(@digit_mapping, :error, fn {word, value} ->
    ...
    extract_numbers(rest, [digit | acc])
  end
end
